...
public class PagingUtil{
	public static String pagingBS4(int totalRecordCount,
			int pageSize, int blockPage, int nowPage, String
			pageName) {
		...
		if(intTemp != 1) {
			pagingStr += "<li class='page-item'><a "
			+"href='"+pageName+"nowPage=1' class='page-link'><i "
			+"class='fas fa-angle-double-left'></i></a></li>";
			pagingStr += "<li class='page-item'><a "
			+"href='"+pageName+"nowPage="+(intTemp-1)+"' class='page-link'>"
		...
		{
			if(intTemp==nowPage) {
				pagingStr += "<li class='page-item active'><a "
				+"href='#' class='page-link'>"+intTemp+"</a></li>";		
			}
			else {
				pagingStr += "<li class='page-item'><a "
				+"href='"+pageName+"nowPage="+intTemp+"'"
		...
		if(intTemp <= totalPage) {
			pagingStr += "<li class='page-item'><a "+
			"href='"+pageName+"nowPage="+intTemp+"' "+
			"class='page-link'><i class='fas "+
			"fa-angle-right'></i></a></li>";
			
			pagingStr += "<li class='page-item'><a "+
			"href='"+pageName+"nowPage="+totalPage+"' "+
			"class='page-link'><i class='fas"+
	...
	public static String pagingImg(int totalRecordCount,
		...
		*/
		...
		// 1 ~ 5 페이지인 경우 이전 블럭이 존재하지 않으므로 아이콘을 숨김처리한다.
		if(intTemp != 1) {
			// 무조건 첫 페이지로 (페이지 번호를 1로 설정한다.)
		...
		while(blockCount<=blockPage && intTemp<=totalPage)
		{
			// 페이지 바로가기 (현재 BLOCK_PAGE 가 5로 설정되므로 5개씩 페이지 번호가 출력 된다.)
			if(intTemp==nowPage) {
				pagingStr += "&nbsp;"+intTemp+"&nbsp;";
			}
			else {
	...
	public static String pagingTxt(int totalRecordCount,
		...
		/*
			첫 번째 페이지 블럭이라면 이전 블럭이 존재하지 않으므로 화면상에 표시하지 않는다.
			두 번째 블럭부터 표시한다.		 
		*/
		if(intTemp != 1) {
		...
		int blockCount = 1;
		// 각 페이지 바로가기. BLOCK_PAGE 의 설정값 만큼 반복한다.
		/*
			BLOCK_PAGE 의 설정값 만큼 반복하는 것을 기본으로 하지만
			전체 페이지를 넘어갈 수는 없으므로 남은 페이지 만큼만 
			반복하기 위해 두 번째 조건을 추가한다.
		*/
		while(blockCount<=blockPage && intTemp<=totalPage)
		{
			// 페이지 바로가기 (현재 BLOCK_PAGE 가 5로 설정되므로 5개씩 페이지 번호가 출력 된다.)
			if(intTemp==nowPage) {
				// 현재 페이지인 경우에는 링크는 제거한다.
				pagingStr += "&nbsp;"+intTemp+"&nbsp;";
			}
			else {
				// 현재 페이지가 아닐 때는 링크가 있어야 한다.
				pagingStr += "&nbsp;";
				pagingStr += "<a href='"+pageName+"nowPage="+intTemp+"'>"+intTemp+"</a>";
				pagingStr += "&nbsp;";
			}
		...
		if(intTemp <= totalPage) {
			...
			pagingStr += "&nbsp;";
			// 마지막 페이지로 바로가기
			pagingStr += "<a href='"+pageName+"nowPage="+totalPage+"'>[마지막 블록으로]</a>";
		}
