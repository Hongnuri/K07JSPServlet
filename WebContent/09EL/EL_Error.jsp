...
<body>
	<fieldset>
		...
		<!-- 
			문자열 형태로 숫자가 전달 되면 연산시 자동으로 숫자로 변경 된 후 연산 된다.
			숫자로 변경할 수 없는 문자가 포함된다면 예외가 발생한다.
			또한 , 문자열끼리의 연결도 EL 에서는 허용되지 않는다.
		-->
		...
		<!-- 
			EL 에서 속성에 접근시 - 이 포함 된 경우에는 .으로 표현 할 수 없고
			반드시 [ 를 통해 표현해야한다.
		-->
		...
		<%
			/*
				String형 배열을 선언한 후 , 각 요소들을 List 컬레션에 삽입
			*/
			...
			for(String mountain : mountains)
					list.add(mountain);
		%>
		<!-- JSTL 의 set 태그를 통해 변수를 생성 및 값을 할당한다. 
				해당 변수는 page 영역에 저장 된다. -->
		<c:set var="elArray" value="<%=mountains %>" />
		<c:set var="eCollection" value="<%=list %>" />
		
		<!-- 
			배열이나 컬렉션 처럼 인덱스로 요소에 접근하는 경우에도 .은 사용할 수 없다.
		-->
		<ul>
		...
		<h4>배열</h4>
		<!-- 
			JSTL 의 forEach 태그를 통해 반복 출력 한다.
			해당 문장은 배열이나 컬렉션의 크기 만큼 반복하는 확장 for 문 형태로 기술되었다.
		-->
		...
		<!-- 
			인덱스를 초과하면 예외가 발생되어야 하지만  EL 에서는
		...
		<ul>
			...
			<!-- MemberDTO 에 getId() 를 주석처리한 후 , 테스트 해볼 것 -->
			<li>속성은 존재 , Getter 가 없는 경우 : ${eldto.id}
				:
				\${eldto.id} [500에러발생]
			</li>
			<li>el변수명이 틀린 경우 : \${noeldto.pwd}
				:
				${nodeldto.pwd} [에러NO , 미출력]</li>		
		</ul>
		...
		<ul>
			...
			<!-- 
				EL 에서는 영역에 접근할 때 , xxScope 계열의 내장객체를 사용한다.
				pageContext 는 JSP 에서 사용하는 내장객체이므로 에러가 발생한다.
			-->	
