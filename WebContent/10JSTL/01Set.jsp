...
<body>
	<!-- 
		...
		scope : 영역을 지정하여 변수를 지정함.
			4가지 영역을 지정할 수 있고 , 기본 값은 가장 좁은 page 여역
		즉 , set 태그를 사용하여 변수를 저장하면 지정 된 영역에 저장된다.
	-->
	<h2>set태그</h2>
	<h3>set태그로 EL에서 사용 할 변수 설정하기</h3>
	<ul>
		<li>
			1. value 속성에 직접 값 대입 :
	...
	<ul>
		...
		<li>
			expVar : <%=pageContext.getAttribute("exVar") %>
		</li>
		<li>
	...
	<h3>EL 로 출력</h3>
	<!-- 
		EL 로 영역의 속성을 읽어올 때는 xxScope 를 생략하면 가장 좁은 영역인 page 영역을 읽어오게 되므로
	...
	<h2>set 태그로 자바빈 객체 설정</h2>
	<h3>인자생성자로 설정</h3>
	<!-- 인자생성자를 통해 DTO 객체를 생성한 후 request 영역에 속성을 저장한다. -->
	<c:set var="argsMember"
		value='<%=new MemberDTO("KIM" , "1111" , "김태희" , null) %>'
		scope="request" />
		
		<h3>JSP 코드로 출력</h3>
		<!-- 영역에 저장 될 때 Object 타입으로 자동형 변환 되므로 사용시
		...
		<!-- 
			EL 로 출력시에는 별도의 형변환이 필요 없고 , 속성명이 중복되지 않는다면 xxScope 계열의 내장객체를 생략할 수 있다.
		-->
		<h3>EL로 출력</h3>
		<ul>
		...
		<!-- 
			set 태그의 target , property 속성은 자바빈 객체의 속성 혹은
			컬렉션의 값을 설정 할 때 , 사용하는 속성이다.
			target : 변수값을 지정하는 속성. 반드시 EL , 표현식만 가능하다.
			property : 값을 지정하는 속성. 표현식 , EL , 값 모두 가능하다.
				- 자바인 경우 : 속성명 (멤버변수)
				- Map 컬렉션인 경우 : key 값
			scope 속성은 var 속성이 지정 된 태그에서만 지정 가능
			target 과 property 를 사용해서 자바빈이나 컬렉션에 값을 설정할 때는 var 속성을 지정할 수 없다.	
		-->
		<h3>target속성과 property속성 사용하기</h3>
		<!-- 1. 변수 설정(new 를 통해 객체를 만들어 setAttribute() 으로 영역에 저장 -->
		<c:set var="defaultMember" value="<%=new MemberDTO() %>"
		scope="request" />
		<!-- 2. 생성 된 변수의 멤버변수 값 지정(setter() 를 통한 값 설정 -->
		...
		<!-- 
			List 계열의 컬렉션은 배열처럼 순서를 보장하고 , 인덱스로 접근 되므로 객체에 접근시에는 인덱스를 사용한다.
		-->		
		<h2>List 컬렉션 이용하기</h2>
		<%
		...
		<!-- 
			0번 인덱스에 저장 된 defaultMember 객체를 지정하여 정보를 출력한다.
		-->
		...
		<!-- 
			Map계열이 컬렉션은 key 를 통해 value 에 접근한다.
			key 를 통한 접근이 index 를 통한 접근 보다 속도가 빠르다.
		-->
		<h2>Map 컬렉션 이용하기</h2>
		<%
		Map map = new HashMap();
